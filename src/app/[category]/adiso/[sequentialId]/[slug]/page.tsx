...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { category, sequentialId, slug } = await params;
  ...
  try {
    ...
    return {
      title: `${publication.title} - ${category.charAt(0).toUpperCase() + category.slice(1)}`,
      description: publication.description || `Adiso de ${category}: ${publication.title}`,
      openGraph: {
...
export default async function CategoryAdisoPage({ params }: PageProps) {
  const { category, sequentialId, slug } = await params;
  ...
  try {
    ...
    if (publication.categorySlug !== category) {
      notFound();
    }
    ...
    return (
      <DedicatedPublicationPage 
        ...
        showBackButton={true}
        backButtonUrl={`/${category}`}
        backButtonText={`Volver a ${category.charAt(0).toUpperCase() + category.slice(1)}`}
      />
    );
  } catch (error) {
