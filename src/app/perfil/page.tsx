        ...
        <div className="mb-4">
          ...
          <div className="flex flex-col gap-2">
            {phones.map((phone, idx) => (
              <div key={`phone-${idx}-${phone.value.substring(0, 10).replace(/\s+/g, '-')}`} className="flex items-center gap-2">
      ...
      <section className="mb-8 bg-white/5 rounded-xl p-6 shadow-lg">
        ...
        <div className="flex flex-col gap-2 mb-2">
          {socialLinks.map((link, idx) => (
            <div key={`social-${idx}-${link.type}-${link.url.substring(0, 10).replace(/\s+/g, '-')}`} className="flex items-center gap-2">
      ...
      <section className="mb-8 bg-white/5 rounded-xl p-6 shadow-lg">
        ...
        <div className="space-y-3 mb-4">
          ...
          {endorsements.map((e, i) => (
            <div key={`endorsement-${i}-${e.name.substring(0, 10).replace(/\s+/g, '-')}`} className="bg-slate-800/60 rounded-lg p-3 shadow flex flex-col">
            ...
            {portfolioPreviews.length > 0 && (
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                {portfolioPreviews.map((item, index) => (
                  <div key={`portfolio-${index}-${item.file.name.substring(0, 20).replace(/\s+/g, '-').toLowerCase()}`} className="relative group bg-slate-800/60 rounded-lg p-2 flex flex-col items-center">
            ...
            {verificationPreviews.length > 0 && (
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                {verificationPreviews.map((item, index) => (
                  <div key={`verification-${index}-${item.file.name.substring(0, 20).replace(/\s+/g, '-').toLowerCase()}`} className="relative group bg-slate-800/60 rounded-lg p-2 flex flex-col items-center">
                    {item.preview === 'generic' ? (
                      <DocumentTextIcon className="w-12 h-12 text-slate-400" />
                    ) : (
                      <Image src={item.preview} alt="Preview" width={80} height={80} className="w-full h-20 object-cover rounded" />
