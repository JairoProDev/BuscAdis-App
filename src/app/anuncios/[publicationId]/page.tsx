  ...
  useEffect(() => {
    const fetchPublication = async () => {
      try {
        ...
        if (!success || !data) {
          throw new Error('No se pudo cargar el anuncio después de varios intentos');
        }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          {relatedPublications.length > 0 && (
            <div className="mb-8">
              <h2 className="text-xl font-bold text-gray-900 mb-4">Anuncios similares</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedPublications.slice(0, 4).map((relatedPub) => (
                  <div key={relatedPub.id} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <a href={`/anuncios/${relatedPub.id}-${relatedPub.title?.toLowerCase().replace(/\s+/g, '-').substring(0, 80)}`}>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            ...
            <div className="text-sm text-gray-500">
              ...
              {publication.subsubcategory && (
                <p>Subsubcategoría: {publication.subsubcategory}</p>
              )}
            </div>
          </div>
